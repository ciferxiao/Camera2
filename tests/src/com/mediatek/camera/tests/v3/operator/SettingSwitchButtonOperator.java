...
public class SettingSwitchButtonOperator extends SettingConfigOperator {
    ...
    private boolean switchOn(String settingName) {
        UiObject2 settingItemEntry = Utils.findObject(By.text(settingName));
        if (settingItemEntry == null) {
            LogHelper.d(TAG, "[switchOn] settingItemEntry is null, return");
            return false;
        }
        ...
        settingItemEntry = Utils.findObject(By.text(settingName));
        switchButton = findSwitchButton(settingItemEntry);
    ...
    private boolean switchOff(String settingName) {
        UiObject2 settingItemEntry = Utils.findObject(By.text(settingName));
        if (settingItemEntry == null) {
            LogHelper.d(TAG, "[switchOff] settingItemEntry is null, return");
            return false;
        }
        UiObject2 switchButton = findSwitchButton(settingItemEntry);
        if (switchButton == null) {
