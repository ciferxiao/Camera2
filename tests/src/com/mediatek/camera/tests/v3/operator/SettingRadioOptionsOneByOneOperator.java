...
public class SettingRadioOptionsOneByOneOperator extends SettingRadioButtonOperator {
    ...
    protected boolean isUseOptionTitleAsSummary() {
        return mIsUseOptionTitleAsSummary;
    }
    ...
    private void getAllRadioOptions(List<String> optionTitleList, List<String> optionSummaryList) {
        boolean hasAddRadioInThisLoop = false;

        // scroll down
        int scrollDownTryTimes = 3;
        do {
            ...
            int radioCount = radioList.getChildCount();
        ...
        } while (hasAddRadioInThisLoop == true || scrollDownTryTimes-- > 0);

        // scroll up
        int scrollUpTryTimes = 3;
        do {
            hasAddRadioInThisLoop = false;
            UiObject2 radioList = Utils.findObject(By.res("android:id/list"));
            List<UiObject2> radioLayoutList = radioList.getChildren();
            int radioCount = radioList.getChildCount();
            for (int i = 0; i < radioCount; i++) {
                String radioOption = radioLayoutList.get(i).findObject(
                        By.clazz("android.widget.TextView").res("android:id/title")).getText();
                if (!optionTitleList.contains(radioOption)) {
                    optionTitleList.add(radioOption);
                    hasAddRadioInThisLoop = true;
                    UiObject2 summary = radioLayoutList.get(i).findObject(
            ...
            Utils.scrollUpOnObject(radioList);
        } while (hasAddRadioInThisLoop == true || scrollUpTryTimes-- > 0);
    }

    private String getCurrentSelectedRadioTitle() {
        UiObject2 radioButton = Utils.scrollOnScreenToFind(
