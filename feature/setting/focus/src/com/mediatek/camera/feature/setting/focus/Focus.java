...
public class Focus extends SettingBase implements
    ...
    public void updateModeDeviceState(String newState) {
        ...
        switch (newState) {
            ...
            case ICameraMode.MODE_DEVICE_STATE_RECORDING:
                ...
                String mode = "com.mediatek.camera.feature.mode.slowmotion.SlowMotionMode";
    ...
    public void overrideValues(@Nonnull String headerKey, String currentValue,
        ...
        if (mFocusListener != null && getValue() != null) {
    ...
    public boolean onSingleTapUp(float x, float y) {
        ...
        //add RCS 皮肤镜下和手动对焦模式下屏蔽自动对焦功能 liuxi 20180724 start
        if (mFocusViewController.isManualFocusMode()){
            return false;
        }
        //add RCS 皮肤镜下和手动对焦模式下屏蔽自动对焦功能 liuxi 20180724 end

        if (!checkAfEnv()) {
            return false;
        }
        mIsAutoFocusTriggered = true;
        if (mFocusViewController != null && mFocusViewController.isActiveFocusRunning()) {
        ...
        switch (state) {
            ...
            case ACTIVE_UNFOCUSED:
                ...
                if (mNeedTriggerShutterButton) {
                    if (!isLockActive()) {
                        resetFocusArea();
                        mFocusListener.updateFocusMode("continuous-picture");
                    }
