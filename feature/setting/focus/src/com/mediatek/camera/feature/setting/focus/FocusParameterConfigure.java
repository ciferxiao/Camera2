...
public class FocusParameterConfigure implements ICameraSetting.IParametersConfigure, IFocus
    ...
    private boolean mNeedCancelAutoFocus = false;
        ...
        if (mIsSupportedFocus) {
            if (mCurrentFocusMode != null) {
                if (mSupportedFocusModeList.contains(mCurrentFocusMode)) {
                    applyFocusCapabilities(parameters);
    ...
    public boolean isFocusCanDo() {
        ...
        if (mCurrentFocusMode == null || Parameters.FOCUS_MODE_INFINITY.equals(mCurrentFocusMode)) {
    ...
    private Camera.AutoFocusCallback mAutoFocusCallback = new Camera.AutoFocusCallback() {
        ...
        public void onAutoFocus(boolean success, Camera camera) {
            long frameNumber = -1;
            synchronized (mLock) {
                if (mFocusStateListener == null || !mIsAutoFocusTriggered) {
                    LogHelper.w(TAG, "[onAutoFocus] mFocusStateListener = " + mFocusStateListener +
                            ",mIsAutoFocusTriggered = " + mIsAutoFocusTriggered);
                    return;
                }
    ...
    private boolean needApplyFocusCapabilities() {
        boolean result = FOCUS_MODE_CONTINUOUS_PICTURE.equals(mCurrentFocusMode) ||
                FOCUS_MODE_CONTINUOUS_VIDEO.equals(mCurrentFocusMode) ||
                (FOCUS_MODE_AUTO.equals(mCurrentFocusMode)
                        && mIsAutoFocusTriggered && !mNeedCancelAutoFocus);
        return result;
    }

    private void applyFocusCapabilities(Parameters parameters) {
        if (!needApplyFocusCapabilities()) {
            LogHelper.w(TAG, "[applyFocusCapabilities] no need update focus capabilities " +
                    "mCurrentFocusMode = " + mCurrentFocusMode + ",mIsAutoFocusTriggered " +
                    mIsAutoFocusTriggered + ",mNeedCancelAutoFocus = " + mNeedCancelAutoFocus);
            return;
        }
        if (mFocusAreaSupported) {
    ...
    private void updateAfCallback(CameraProxy cameraProxy, boolean isContinueAf) {
        LogHelper.d(TAG, "[updateAfCallback] + isContinueAf " + isContinueAf + "," +
                "mIsAutoFocusTriggered = " + mIsAutoFocusTriggered + " ,mNeedCancelAutoFocus " +
                mNeedCancelAutoFocus);
        if (isContinueAf) {
            if (isMzafEnabled()) {
