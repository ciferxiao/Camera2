...
public class DualZoom extends SettingBase {
    ...
    public void addViewEntry() {
        ...
        if (isGestureSupported()) {
            mDualViewCtrl.setSeekBarChangeListener(mSeekBarChangeListener);
            mAppUi.registerGestureListener(mZoomGestureImpl, IApp.DEFAULT_PRIORITY);
        } else {
            mDualViewCtrl.setSeekBarChangeListener(null);
        }
    ...
    private SeekBar.OnSeekBarChangeListener mSeekBarChangeListener
        ...
        public void onStartTrackingTouch(SeekBar seekBar) {
            if (mZoomConfig != null) {
                mDualViewCtrl.clearInvalidView(true);
                mZoomConfig.onScaleStatus(false, false);
            }
        }

        @Override
        public void onProgressChanged(SeekBar seekBar, int gear, boolean var3) {
            String curValue = getValue();
            if (IDualZoomConfig.ZOOM_OFF.equals(curValue) || mPreviousSpan != 0) {
                return;
            }
            double ratio;
