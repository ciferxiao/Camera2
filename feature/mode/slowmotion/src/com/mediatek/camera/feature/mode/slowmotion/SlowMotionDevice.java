...
public class SlowMotionDevice implements IDeviceController,
            ...
            if (mSession != null) {
                synchronized (mSession) {
                    if (mSession != null) {
                        ...
                        slowMoRequests = mSession.createHighSpeedRequestList(builder.build());
                        mSession.setRepeatingBurst(
                                slowMoRequests, mPreviewCapProgressCallback, null);
                    }
                }
            }
    ...
    public void updatePreviewSurface(SurfaceHolder surfaceHolder) {
        LogHelper.d(TAG, "[updatePreviewSurface] surfaceHolder = " + surfaceHolder);
    ...
    public void startRecording() {
        ...
        try {
            ...
            configAeFpsRange(builder, mProfile, true);
            mSession.setRepeatingBurst(mSession.createHighSpeedRequestList(builder.build()),
    ...
    private void repeatingPreview() {
        ...
        CaptureRequest.Builder builder;
        try {
            builder = mCamera2Proxy.createCaptureRequest(Camera2Proxy.TEMPLATE_PREVIEW);
            mFirstFrameArrived = false;
            builder.addTarget(mPreviewSurface);
            mSettingDevice2Configurator.configCaptureRequest(builder);
            configAeFpsRange(builder, mProfile, false);
            List<CaptureRequest> slowMoRequests = null;
            slowMoRequests = mSession.createHighSpeedRequestList(builder.build());
            mSession.setRepeatingBurst(slowMoRequests, mPreviewCapProgressCallback, null);
        } catch (CameraAccessException e) {
            e.printStackTrace();
        }
