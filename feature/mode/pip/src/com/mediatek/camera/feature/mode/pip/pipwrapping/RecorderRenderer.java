...
public class RecorderRenderer extends Renderer implements Runnable {
    ...
    private boolean mIsStopRecordingReceived = false;
    private int mSwapVideoBufferCount = 0;
    private ConditionVariable mStartConditionVariable = new ConditionVariable();
    ...
    /**
     * Construct an instance of RecorderRenderer.
    ...
    /**
     * Init recorder renderer, should be called in GL Thread.
     */
     ...
     * @param surface the recording surface.
     */
    public void setRecordingSurface(Surface surface) {
        LogHelper.d(TAG, "setRecordingSurface surface = " + surface);
        if (surface == null) {
            throw new NullPointerException("setRecordingSurface, but surface is null!!!");
        }
        ...
        synchronized (mReadyFence) {
            ...
            while (!mReady) {
                try {
                    LogHelper.d(TAG, "wait for recording thread ready, current mReady = " + mReady);
                    mReadyFence.wait();
    ...
    public void startRecording() {
        ...
        synchronized (mReadyFence) {
            ...
            mStartConditionVariable.block(200);
    ...
    /**
     * when new frame is available, swap buffer to media recorder.(Call from GL
     * thread.
    ...
    public void draw(int textureId, long timestamp) {
        synchronized (mReadyFence) {
            ...
            if (mEncoderHandler != null) {
                ...
                if (mIsStopRecordingReceived && (mSwapVideoBufferCount > 2)) {
                    mReady = false; // when stop received, do not receive frame next time
                    mIsStopRecordingReceived = false;
    ...
    public void stopRecording() {
        LogHelper.i(TAG, "stopRecording mReady = " + mReady + " mIsStopRecordingReceived = "
                + mIsStopRecordingReceived);
        mStopConditionVariable.close();
        synchronized (mReadyFence) {
            // make mReady to true to ensure receive a frame
            mReady = true;
            mIsStopRecordingReceived = true;
        }
