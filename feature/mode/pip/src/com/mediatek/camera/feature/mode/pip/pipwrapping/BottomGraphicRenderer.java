...
/**
 * Bottom graphic's renderer.
 */
public class BottomGraphicRenderer extends Renderer {
    private static final Tag TAG =
            new Tag(BottomGraphicRenderer.class.getSimpleName());
    // position
    ...
    /**
     * Construct a BottomGraphicRenderer.
    ...
    public void setRendererSize(int width, int height, boolean needReverse) {
        LogHelper.d(TAG, "setRendererSize width = " + width +
                " height = " + height +
    ...
    /**
     * Draw bottom graphic frame.
     * @param preTexId the preview texture id.
     * @param preTexMtx the preview texture matrix.
     * @param texReverseRotateMtx the reverse rotate matrix.
    ...
    public void draw(int preTexId, final float[] preTexMtx, final float[] texReverseRotateMtx,
        ...
        mTexCoordinateBuf = createFloatBuffer(mTexCoordinateBuf,
                GLUtil.createTexCoordinate(0, 1, 0, 1, needFlip));
        mTexCoordinateBuf.position(0);
        GLES20.glVertexAttribPointer(mTexCoordinateHandle, 2,
                GLES20.GL_FLOAT, false, 4 * 2, mTexCoordinateBuf);
        ...
        //sampler
        GLES20.glUniform1i(
                (preTexMtx == null) ? mPictureSampleHandle :
                        mPreviewSamplerHandle, sampleId);
        //texture
