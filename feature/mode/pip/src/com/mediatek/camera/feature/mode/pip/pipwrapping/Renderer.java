...
public class Renderer {
    ...
    private static ConcurrentHashMap<Integer, ArrayList<Integer>> sResourceContourMap =
     ...
     */
    protected FloatBuffer createFloatBuffer(FloatBuffer floatBuffer,
                                            float[] texCoordinate) {
        if (floatBuffer == null) {
     ...
     * @param needReverse whether need reverse.
     */
     ...
     * @param scaled need scaled.
     * @param rotated need rotated.
     */
    ...
    protected int initBitmapTexture(int resourceId, boolean needCropSmallBox) throws IOException {
        ...
        try {
            ...
            if (needCropSmallBox) {
                ...
                if (!sResourceContourMap.containsKey(resourceId)) {
                    int minX = width;
                    int maxX = 0;
                    int minY = height;
                    int maxY = 0;
                    ...
                    LogHelper.v(TAG, "read consume " + (System.currentTimeMillis() - begin)
                            + "ms bitmap width = " + width + " bitmap height = " + height
                    ...
                    contour.add(minY);
                    contour.add(edge);
                    sResourceContourMap.put(resourceId, contour);
                }
                int minX = sResourceContourMap.get(resourceId).get(0);
                int minY = sResourceContourMap.get(resourceId).get(1);
                int edge = sResourceContourMap.get(resourceId).get(2);
                float xRatio = (minX + (float) edge / 2 - (float) width / 2) / (float) width;
                float yRatio = (minY + (float) edge / 2 - (float) height / 2) / (float) height;
    ...
    /**
     * Create program.
     * @param vertexSource the vertex source code.
    ...
    class CropBox {
        ...
        /**
         * Get translate x ratio.
         * @return the translate x ratio.
         */
        ...
        /**
         * Set translate y ratio.
         * @param yRatio the translate y ratio.
         */
        ...
        /**
         * Get translate y ratio.
         * @return the translate y ratio.
         */
