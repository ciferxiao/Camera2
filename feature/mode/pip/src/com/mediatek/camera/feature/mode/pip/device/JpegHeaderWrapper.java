...
public class JpegHeaderWrapper {
    private static final Tag TAG = new Tag(JpegHeaderWrapper.class.getSimpleName());
    ...
    /**
     * Read jpeg header.
     * @param jpeg the jpeg buffer.
     * @return the header.
     * @throws IllegalArgumentException the illegal argument exception.
     * @throws IOException the IO exception.
     */
    public byte[] readJpegHeader(byte[] jpeg) throws IllegalArgumentException, IOException {
        ...
        inStream.close();
        LogHelper.d(TAG, "readJpegHeader jpegHeader length = " + jpegHeader.length +
                ",readLength = " + readLength + ",jpegHeaderLength = " + jpegHeaderLength);
        return jpegHeader;
    }
     ...
     * @throws IOException if write with error, throws IO exception.
     */
    public byte[] writeJpegHeader(byte[] jpeg, byte[] header) throws IOException {
        ...
        try {
            ...
            LogHelper.d(TAG, "[writeJpegHeader]jpegHeaderLength = " + jpegHeaderLength
            ...
            LogHelper.d(TAG, "[writeJpegHeader]read raw jpage data length = " + jpegdata.length
            ...
            LogHelper.e(TAG, "[writeJpegHeader]exception " + e.toString());
        } finally {
            inStream.close();
            out.close();
        }
    ...
    private int readJpegHeaderLength(byte[] jpeg) throws IOException {
        if (jpeg == null) {
            LogHelper.e(TAG, "[readJpegHeaderLength]jpeg is null!");
            throw new IllegalArgumentException("Argument is null");
        }
        InputStream inStream = new ByteArrayInputStream(jpeg);
        CountedDataInputStream dataStream = new CountedDataInputStream(inStream);
        if (dataStream.readShort() != SOI) {
            LogHelper.e(TAG, "[readJpegHeaderLength]Invalid Jpeg Format!");
            throw new IOException("Invalid Jpeg Format");
        }
        ...
        while (marker != EOI && !isSofMarker(marker)) {
            ...
            LogHelper.d(TAG, "Read marker = " + Integer.toHexString(marker) + " jpegHeaderLength = "
                    + jpegHeaderLength);
            if (markerLength < 2 || (markerLength - 2) != dataStream.skip(markerLength - 2)) {
                LogHelper.e(TAG, "[readJpegHeaderLength]Invalid Marker Length = " + markerLength);
                throw new IOException("Invalid Marker Length = " + markerLength);
            }
