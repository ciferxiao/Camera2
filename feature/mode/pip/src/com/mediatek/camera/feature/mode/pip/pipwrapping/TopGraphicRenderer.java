...
public class TopGraphicRenderer extends Renderer {
    ...
    private FloatBuffer mTempTexCoordinateBuf;
    // matrix
    ...
    private int mTempTexCoordinateHandle = -1;
    private int mPosMtxHandle = -1;
    private int mTexMtxHandle = -1;
    private int mTexRotateMtxHandle = -1;
    private int mIsPreviewHandle = -1;
    private int mPictureSampleHandle = -1;
    private int mPreviewSamplerHandle = -1;
    private int mBackTempSamplerHandle = -1;
    ...
    /**
     * Construct a top graphic renderer.
    ...
    /**
     * This will be called after makeCurrent.
     */
    public void init() {
        initProgram();
        mTexCoordinateBuf = createFloatBuffer(mTexCoordinateBuf, GLUtil.createTexCoordinate());
        mTempTexCoordinateBuf = createFloatBuffer(mTempTexCoordinateBuf,
                GLUtil.createTexCoordinate());
        mTopTemplateRenderer.init();
    }

    /**
     * Init template texture.
     * @param backResource back resource id.
     * @param frontResource front resource id.
     */
    public void initTemplateTexture(int backResource, int frontResource) {
        if (backResource < 0 || frontResource < 0 || mBackTempTexId == backResource) {
            return;
        }
        if (mBackTempTexId > 0) {
    ...
    /**
     * Draw top frame.
     * @param preTex the preview texture id.
     * @param preTexMtx the preview texture matrix.
     * @param texReverseRotateMtx texture reverse rotate matrix.
     * @param topRect top graphic rect.
     * @param rotation the rotation.
     * @param needFlip whether need flip.
     * @param bottomIsMainCam bottom is main camera.
     */
    public void draw(int preTex, final float[] preTexMtx, final float[] texReverseRotateMtx,
            final AnimationRect topRect, int rotation, boolean needFlip, boolean bottomIsMainCam) {
        ...
        animationRect.rotate(topRect.getCurrentRotationValue());
        ...
        animationRect.rotate(animationRect.getCurrentRotationValue());
        // scale by the same ratio to find the smallest box to wrap the small
        // box
        animationRect.scale(cropBox.getScaleRatio(), false);
        animationRect.rotate(animationRect.getCurrentRotationValue(), centerX, centerY);
        ...
        float lowWidth = .0f;
        float highWidth = .0f;
        float topGraphicCropValue =
                PipOperator.PIPCustomization.TOP_GRAPHIC_CROP_RELATIVE_POSITION_VALUE;
        if (rotation < 0) {
            ...
            lowWidth = isLandScape ? (longer - shorter)
                    * (bottomIsMainCam ? (1f - topGraphicCropValue) :
            ...
            highWidth = isLandScape ? ((longer - shorter)
                    * (bottomIsMainCam ? (1f - topGraphicCropValue) :
                    ...
                    : 1;
            mTexCoordinateBuf = createFloatBuffer(mTexCoordinateBuf,
                    GLUtil.createTexCoordinate(
                            lowWidth,
                            highWidth,
                            lowHeight,
            ...
            case 0:
                mTexCoordinateBuf = createFloatBuffer(mTexCoordinateBuf,
                        GLUtil.createReverseStandTexCoordinate(
                                lowWidth, highWidth, lowHeight, highHeight));
                break;
            case 90:
                mTexCoordinateBuf = createFloatBuffer(mTexCoordinateBuf,
                        GLUtil.createRightTexCoordinate(
                                lowWidth, highWidth, lowHeight, highHeight));
                break;
            case 180:
                mTexCoordinateBuf = createFloatBuffer(mTexCoordinateBuf,
                        GLUtil.createStandTexCoordinate(
                                lowWidth, highWidth, lowHeight, highHeight));
                break;
            case 270:
                mTexCoordinateBuf = createFloatBuffer(mTexCoordinateBuf,
                        GLUtil.createLeftTexCoordinate(
                                lowWidth, highWidth, lowHeight, highHeight));
                break;
            default:
                break;
        ...
        GLES20.glVertexAttribPointer(mTexCoordinateHandle, 2, GLES20.GL_FLOAT, false, 4 * 2,
                mTexCoordinateBuf);
        mTempTexCoordinateBuf.position(0);
        GLES20.glVertexAttribPointer(mTempTexCoordinateHandle, 2, GLES20.GL_FLOAT, false, 4 * 2,
                mTempTexCoordinateBuf);
        GLES20.glEnableVertexAttribArray(mPositionHandle);
        GLES20.glEnableVertexAttribArray(mTexCoordinateHandle);
        GLES20.glEnableVertexAttribArray(mTempTexCoordinateHandle);
        // draw
        // matrix
        ...
        GLES20.glUniform1f(mIsPreviewHandle, (preTexMtx == null) ? 0.0f : 1.0f);
        // sampler
        GLES20.glUniform1i((preTexMtx == null) ? mPictureSampleHandle : mPreviewSamplerHandle, 0);
        GLES20.glUniform1i(mBackTempSamplerHandle, 1);
        // texture
    ...
    private void initProgram() {
        ...
        mTempTexCoordinateHandle = GLES20.glGetAttribLocation(mProgram, "aTempTexCoord");
        // matrix
        ...
        mIsPreviewHandle = GLES20.glGetUniformLocation(mProgram, "uIsPreview");
        // sampler
        mPreviewSamplerHandle = GLES20.glGetUniformLocation(mProgram, "uPreviewSampler");
        mBackTempSamplerHandle = GLES20.glGetUniformLocation(mProgram, "uBackSampler");
        mPictureSampleHandle = GLES20.glGetUniformLocation(mProgram, "uPictureSampler");
    }
