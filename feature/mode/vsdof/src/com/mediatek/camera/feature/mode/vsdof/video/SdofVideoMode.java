        ...
        if (isSupportedExtendInfo()) {
            try {
                ...
                boolean isRecordingAudio = "on".equals(
                        mSettingManager.getSettingController()
                                .queryValue(mVideoHelper.MICROPHONE_KEY));
                if (isRecordingAudio && mVideoHelper.isHdRecordingEnabled()) {
                    MediaRecorderEx.setHDRecordMode(mRecorder.getMediaRecorder(),
                            mVideoHelper.getRecordMode(
                                    mSettingManager.getSettingController().queryValue(
                                            mVideoHelper.AUDIO_MODE_KEY), true), true);
                }
