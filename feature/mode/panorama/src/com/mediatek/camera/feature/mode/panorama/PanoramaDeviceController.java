...
class PanoramaDeviceController implements IPanoramaDeviceController,
    ...
    public void updatePreviewSurface(SurfaceHolder surfaceHolder) {
        mRequestHandler.obtainMessage(UPDATE_PREVIEW_SURFACE, surfaceHolder).sendToTarget();
    }
    ...
    private void doUpdatePreviewSurface(SurfaceHolder surfaceHolder) {
        LogHelper.d(TAG, "doUpdatePreviewSurface,surfaceHolder = " + surfaceHolder + ",state : " +
        ...
        synchronized (mSurfaceHolderSync) {
            ...
            if (isStateReady && mCameraProxy != null) {
                if (surfaceHolder != null) {
                    doStartPreview(surfaceHolder, mCameraProxy.getParameters());
                } else {
                    doStopPreview();
                }
            }
    ...
    private void doCloseCamera(boolean sync) {
        ...
        if (CameraState.CAMERA_UNKNOWN == mCameraState) {
            LogHelper.d(TAG, "[doCloseCamera]+, camera have closed,return");
            return;
        }
        mWaitOpened.block();
    ...
    private void doStartPreview(SurfaceHolder surfaceHolder, Parameters parameters) {
    ...
    private class CameraDeviceProxyStateCallback extends StateCallback {
        ...
        public void onOpened(@Nonnull CameraProxy cameraProxy) {
            LogHelper.i(TAG, "[onOpened],proxy = " + cameraProxy +
                    ",mSurfaceHolder = " + mSurfaceHolder + ",state = " + mCameraState);
            ...
            synchronized (mSurfaceHolderSync) {
                ...
                if (mSurfaceHolder != null) {
                    doStartPreview(mSurfaceHolder, parameters);
                }
            }
            mCameraOpenCloseLock.release();
            mWaitOpened.open();
