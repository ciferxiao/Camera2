...
import com.mediatek.camera.rcs.RcsOrganManager;
import com.mediatek.camera.rcs.RcsPatientManager;
import com.mediatek.camera.rcs.RcsShotManager;
import com.mediatek.camera.rcs.RcsStateBarManager;
...
public class CameraAppUI implements IAppUi {
    ...
    private RcsOrganManager mRcsOrganManager;
    private RcsShotManager mRcsShotManager;
    private RcsPatientManager mRcsPatientManager;
    private RcsStateBarManager mRcsStateBarManager;
    ...
    public void onCreate() {
        ...
        Log.i("leftmode"," mApp.isLeftHandMode() == " + mApp.isLeftHandMode());
        if (mApp.isLeftHandMode()){
            parentView = (ViewGroup) mApp.getActivity().getLayoutInflater()
                    .inflate(R.layout.rcs_left_camera_ui_root, rootView, true);
        }else{
            parentView = (ViewGroup) mApp.getActivity().getLayoutInflater()
                    .inflate(R.layout.rcs_right_camera_ui_root, rootView, true);
        }
        ...
        if (!(MediaStore.ACTION_IMAGE_CAPTURE.equals(action)
            ...
            //mViewManagers.add(mThumbnailViewManager);//M: Rcs 屏蔽View liuxi start 20180713
            mThumbnailViewManager.setVisibility(View.GONE);
        }
        ...
        mModePickerManager.setVisibility(View.GONE);//M: Rcs 屏蔽View liuxi start 20180713
        //mViewManagers.add(mModePickerManager);

        mQuickSwitcherManager = new QuickSwitcherManager(mApp, parentView);
        mQuickSwitcherManager.setVisibility(View.GONE);//M: Rcs 屏蔽View liuxi start 20180713
        mViewManagers.add(mQuickSwitcherManager);

        mIndicatorViewManager = new IndicatorViewManager(mApp, parentView);
        mIndicatorViewManager.setVisibility(View.GONE);//M: Rcs 屏蔽View liuxi start 20180713
        ...
        mEffectViewManager.setVisibility(View.GONE);//M: Rcs 屏蔽View liuxi start 20180713
        mViewManagers.add(mEffectViewManager);

        mOnScreenHintManager = new OnScreenHintManager(mApp, parentView);


        mRcsOrganManager = new RcsOrganManager(mApp,parentView);
        mRcsOrganManager.setVisibility(View.VISIBLE);
        mViewManagers.add(mRcsOrganManager);

        mRcsShotManager = new RcsShotManager(mApp,parentView);
        mRcsShotManager.setVisibility(View.VISIBLE);
        mViewManagers.add(mRcsShotManager);

        mRcsPatientManager = new RcsPatientManager(mApp,parentView);
        mRcsPatientManager.setVisibility(View.VISIBLE);
        mViewManagers.add(mRcsPatientManager);

        mRcsStateBarManager = new RcsStateBarManager(mApp,parentView);
        mRcsStateBarManager.setVisibility(View.VISIBLE);
        mViewManagers.add(mRcsStateBarManager);
    ...
    public void onResume() {
        ...
        FrameLayout root = (FrameLayout) mApp.getActivity().findViewById(R.id.app_ui);
        ...
        mRcsOrganManager.registerView();//add RCS SeekBar和器官View的添加 liuxi 20180716
        mRcsShotManager.registerView();//add RCS 拍照和对焦View的添加 liuxi 20180716
        mRcsPatientManager.registerView();//add RCS 患者信息View的添加 liuxi 20180716
        mRcsStateBarManager.registerView();//add RCS 电池,时间View的添加 liuxi 20180716
    ...
    public void updateCurrentMode(final String modeEntry) {
        ...
        if (mModeProvider != null) {
            ...
            Log.i("dddddddd","item.mModeName = " + item.mModeName);
            mCurrentModeName = item.mModeName;
            Log.i("dddddddd","mCurrentModeName = " + mCurrentModeName);
    ...
    public void applyAllUIVisibilityImmediately(int visibility) {
        ...
        //add RCS liuxi 录像模式下要显示的View 20180723 start
        mRcsStateBarManager.setVisibility(View.VISIBLE);
        mRcsShotManager.setVisibility(View.VISIBLE);
        mRcsOrganManager.setVisibility(View.VISIBLE);
        mRcsPatientManager.setVisibility(View.VISIBLE);
        //add RCS liuxi 录像模式下要显示的View 20180723 end
    }
    ...
    @Override
    public void onRefreshVideoResource(boolean isStartRecording) {
        mRcsShotManager.setVideoImageResource(isStartRecording);
    }
    ...
    private class OnShutterChangeListenerImpl implements
        ...
        public void onShutterTypeChanged(String newShutterType) {
            ...
            LogHelper.i(TAG, "onShutterTypeChanged mCurrentModeName " + mCurrentModeName);
            ModeItem item = mModeProvider.getModeEntryName(mCurrentModeName, mCurrentModeType);
            LogHelper.i(TAG, "onShutterTypeChanged item " + item);
        ...
        public boolean onTouch(View view, MotionEvent motionEvent) {
            if (mGestureManager != null) {
                ...
                if (config.orientation == Configuration.ORIENTATION_PORTRAIT) {
                    ...
                    //add Rcs 强制横屏之后,对焦不能点的问题 liuxi start 20180713 start
                    ...
                    //add Rcs 强制横屏之后,对焦不能点的问题 liuxi start 20180713 end
                }
    ...
    private void configUIEnabled(int module, boolean enabled) {
        ...
        switch (module) {
            ...
            case THUMBNAIL:
                if (mThumbnailViewManager != null) {
                    mThumbnailViewManager.setEnabled(false);
                }
                break;
            case SHUTTER_BUTTON:
    ...
    public RcsOrganManager getRcsOrganManager() {
        return mRcsOrganManager;
    }

    public RcsShotManager getRcsShotManager() {
        return mRcsShotManager;
    }
    ...
    public RcsStateBarManager getRcsStateBarManager() {
        return mRcsStateBarManager;
    }

    public ShutterButtonManager getShutterManager() {
    ...
    public void setPhotoMode(){
        if (mShutterManager!=null){
            mShutterManager.onShutterChangedStart("Picture");
        }
    }

    public void setVideoMode(){
        if (mShutterManager!=null){
            mShutterManager.onShutterChangedStart("Video");
        }
    }
