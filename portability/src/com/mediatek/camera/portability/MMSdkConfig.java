...
public class MMSdkConfig {
    ...
    public static void preSetSession(int cameraId, CaptureRequest builder) {
        IMMSdkService mmsdk;
        try {
            mmsdk = IMMSdkService.Stub.asInterface(
                    ServiceManager.getService("media.mmsdk"));
            Class clazz = mmsdk.getClass();
            Method m = clazz.getDeclaredMethod(
                    "setConfigureParam", int.class, CaptureRequest.class);
            m.setAccessible(true);
            m.invoke(mmsdk, cameraId, builder);
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
