...
class Camera2Handler extends HistoryHandler {
    ...
    private final StateCallback mSessionStateCallback = new StateCallback() {
        ...
        public void onClosed(final CameraCaptureSession session) {
            ...
            if (proxy != null) {
                Runnable runnable = new Runnable() {
                    ...
                    public void run() {
                        ...
                        // need remove the session which is closed.
            ...
            // when the session which is closed is same as current session,need set the current
            // session to null. because when create a new session that the onConfigured() is before
            // the old session onClosed() callback. so must need judge.
