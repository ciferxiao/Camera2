...
public class CameraDeviceManagerImpl extends CameraDeviceManager {
    private static final Tag TAG = new Tag("CamDeviceMgr");
    ...
    public synchronized void openCameraSync(@Nonnull String cameraId,
                                            ...
                                            @Nullable Handler handler) throws CameraOpenException {
        LogHelper.i(TAG, "[openCameraSync]");
        checkDevicePolicy();
        CameraProxyCreatorImpl proxyCreator = mCameraProxyCreatorList.get(cameraId);
        if (proxyCreator == null) {
            proxyCreator = new CameraProxyCreatorImpl(cameraId);
    ...
    @Override
    public synchronized void recycle(@Nonnull String cameraId) {
        LogHelper.i(TAG, "[recycle] id:" + cameraId);
        if (cameraId != null && mCameraProxyCreatorList.containsKey(cameraId)) {
            mCameraProxyCreatorList.remove(cameraId);
        }
    }
            ...
            public void handleMessage(Message msg) {
                ...
                switch (msg.what) {
                    case MSG_OPEN_CAMERA:
                        ...
                        mOpenStateCallback = stateCallback == null ? mDummyCameraStateCallback :
                                                                      stateCallback;

                        if (mCameraProxy == null) {
                            LogHelper.i(mHandlerTag, "[openCamera]+");
                            long startTime = SystemClock.uptimeMillis();
                            FeatureLoader.notifySettingBeforeOpenCamera(mContext,
                                    mCameraId,
                                    CameraApi.API1);
                            retryOpenCamera();
