...
public class VideoMode extends CameraModeBase {
    ...
    public void pause(@Nullable DeviceUsage nextModeDeviceUsage) {
        ...
        Log.d("lx_video", "mApp.isUvcConnected() == pause " + mApp.isUvcConnected() );
        if (mApp.isUvcConnected()){
            if (mCameraDevice!=null){
                mCameraDevice.onPause();
            }
        }
        //xiao add for uvs video
        Log.d("xiao_video", "pause ==========" );
    }
    ...
    public void unInit() {
        ...
        super.unInit();
        Log.d("lx_video", "mApp.isUvcConnected() unInit== " + mApp.isUvcConnected());
        if (mApp.isUvcConnected()){
            if (mCameraDevice!=null){
    ...
    public boolean onCameraSelected(@Nonnull String newCameraId) {
        ...
        if (canSelectCamera(newCameraId)) {
            ...
            Log.i("dddddddd","mCameraDevice ==  " + mCameraDevice);
            Log.i("dddddddd","getPreviewStartCallback() ==  " + getPreviewStartCallback());
    ...
    protected void initCameraDevice(CameraApi api,IApp miapp) {
        mCameraDevice = DeviceControllerFactory.createDeviceCtroller(
                mApp.getActivity(), api, mCameraContext,miapp);
    }

    protected void addFileToMediaStore() {
        //xiao add for video name 
        String videoname = mIApp.getCurrentVideoName();
    
        ContentValues contentValues
                = mVideoHelper.prepareContentValues(true, mOrientationHint, null,videoname);
    ...
    protected boolean isSupportedExtendInfo() {
        boolean isSupport = MediaRecorderEx.isCanUseParametersExtra();
        LogHelper.d(TAG, "[isSupportedExtendInfo] isSupport = " + isSupport);
        return isSupport;
    }

    protected void setMediaRecorderParameters() {
        if (isSupportedExtendInfo()) {
            try {
                int infoNumber = mVideoHelper.MEDIA_INFO.length;
                for (int i = 0; i < infoNumber; i++) {
                    MediaRecorderEx.setParametersExtra(mRecorder.getMediaRecorder(),
                            mVideoHelper.RECORDER_INFO_SUFFIX + mVideoHelper.MEDIA_INFO[i]);
                }
                boolean isRecordingAudio = "on".equals(
                   mSettingManager.getSettingController().queryValue(mVideoHelper.MICROPHONE_KEY));
                if (isRecordingAudio && mVideoHelper.isHdRecordingEnabled()) {
                    MediaRecorderEx.setHDRecordMode(mRecorder.getMediaRecorder(),
                            mVideoHelper.getRecordMode(mSettingManager.getSettingController()
                                    .queryValue(mVideoHelper.AUDIO_MODE_KEY), true), true);
                }
        ...
        public void handleMessage(Message msg) {
            ...
            switch (msg.what) {
                case HANDLER_INIT_DEVICE:
                    initCameraDevice(mCameraApi,mIApp);
                    ...
                    Log.i("lddddd"," mCameraDevice  == " + mCameraDevice);
                    Log.i("lddddd"," mVideoHelper.getPreviewFrameCallback()  == " + mVideoHelper.getPreviewFrameCallback());
                    Log.i("lddddd"," getPreviewStartCallback()  == " + getPreviewStartCallback());
        ...
        if (getVideoState() == VideoState.STATE_RECORDING
            ...
            try {
                ...
                synchronized (mStopObject) {
                    LogHelper.d(TAG, "[stopRecording] stop done notify waiter");
    ...
    private void prepareStartRecording() {
        ...
        mAppUi.onRefreshVideoResource(true);
    ...
    private void prepareStopRecording() {
            ...
            mAppUi.onRefreshVideoResource(false);
    ...
    private boolean isSupportPauseResume() {
        if (isSupportedExtendInfo() || Build.VERSION.SDK_INT > Build.VERSION_CODES.M) {
            return true;
        }
        return false;
    }
    ...
    public void requestChangeExposureValue(int value) {
        if(mCameraDevice!=null){
           mCameraDevice.requestChangeExposureValue(value);
        }
    }
    ...
    private void waitForStop() {
        synchronized (mStopObject) {
            try {
                LogHelper.d(TAG, "[waitForStop] +");
                mStopObject.wait();
                LogHelper.i(TAG, "[waitForStop] -");
        ...
        public void onDataReceived(byte[] jpegData) {
            ...
            if (jpegData != null) {
                ...
                mCameraContext.getMediaSaver().addSaveRequest(jpegData,
                        mVideoHelper.prepareContentValues(false, orientation, size,null),
                        null, mFileSavedListener);
            }
            mCanTakePicture = true;
        }
    };
        ...
        public void onInfo(MediaRecorder mr, int what, int extra) {
            ...
            switch (what) {
                ...
                case VideoHelper.MEDIA_INFO_WRITE_SLOW:
                    ...
                    mVideoHandler.sendEmptyMessage(HANDLER_STOP_RECORDING);
                    break;
                case VideoHelper.MEDIA_INFO_FPS_ADJUSTED:
                case VideoHelper.MEDIA_INFO_BITRATE_ADJUSTED:
                    mVideoUi.showInfo(BAD_PERFORMANCE_DROP_QUALITY);
                    break;
                default:
                    break;
            }
