...
public class VideoHelper {
    ...
    public static final String AUDIO_MODE_KEY = "key_audio_mode";
    ...
    public static final int[] MEDIA_INFO = new int[] {
            MEDIA_INFO_BITRATE_ADJUSTED,
            MEDIA_INFO_CAMERA_RELEASE,
            MEDIA_INFO_FPS_ADJUSTED,
            MEDIA_INFO_START_TIMER,
            MEDIA_INFO_WRITE_SLOW
    };

    private static final String[] PREF_CAMERA_VIDEO_HD_RECORDING_VALUES = {"normal", "indoor"};
    ...
    private String RcsfileDirectory; 
    ...
    public VideoHelper(ICameraContext context, IApp app, IDeviceController cameraDevice) {
        ...
        RcsfileDirectory=mCameraContext.getStorageService().getFileDirectory()
                 + '/' + format.format(new Date());
                 
        newfile = new File(RcsfileDirectory);
        if (!newfile.exists()) {
            newfile.mkdir();
        }
        
        Log.d("xiao_path", "RcsfileDirectory 111 ==   " + RcsfileDirectory);
    }
    /**
     ...
     */
    public ContentValues prepareContentValues(boolean isVideo, int orientation, Size size,String name) {
        mLock.lock();
        try {
            LogHelper.d(TAG, "[prepareContentValues] isVideo = " + isVideo
                    + " orientation = " + orientation);
            android.util.Log.d("xiao222","111111111111111111");
        
            mOrientation = orientation;
            initializeCommonInfo(isVideo,name);
            if (isVideo) {
    ...
    Log.d("xiao_path", "RcsfileDirectory 222 ==   " + RcsfileDirectory);
    
        mTempPath = RcsfileDirectory + 
    ...
    /**
     * Get record mode for audio.
     * @param mode setting value.
     * @param isRecordAudio is can record audio
     * @return result.
     */
    public int getRecordMode(String mode, boolean isRecordAudio) {
        if (isRecordAudio) {
            if (PREF_CAMERA_VIDEO_HD_RECORDING_VALUES[0].equals(mode)) {
                return MediaRecorderEx.HDRecordMode.NORMAL;
            } else if (PREF_CAMERA_VIDEO_HD_RECORDING_VALUES[1].equals(mode)) {
                return MediaRecorderEx.HDRecordMode.INDOOR;
            } else {
                return MediaRecorderEx.HDRecordMode.OUTDOOR;
            }
        } else {
            return MediaRecorderEx.HDRecordMode.NORMAL;
        }
    ...
    private void initializeCommonInfo(boolean isVideo,String videoname) {
        ...
        if(videoname != null){
            mFileName = createFileName(isVideo, videoname);
        }
        Log.d("xiao_path", "RcsfileDirectory 333 ==   " + RcsfileDirectory);
        //xiao add for video save path start
        mFilePath = RcsfileDirectory+ '/' + mFileName;//xiao change for video save path
        android.util.Log.d("xiao111","mFilePath == " + mFilePath);
        //xiao add for video save path end
    }
    ...
    private class SurfaceChangeListener implements IAppUiListener.ISurfaceStatusListener {
        ...
        public void surfaceAvailable(SurfaceHolder surfaceHolder, int width, int height) {
            LogHelper.i(TAG, "[surfaceAvailable] holder = " + surfaceHolder + " width = " + width +
                    " height = " + height);
            mPreviewSize = new Size(width, height);
            if (mCameraDevice != null) {
                mCameraDevice.updatePreviewSurface(surfaceHolder);
            }
        }
        @Override
        public void surfaceChanged(SurfaceHolder surfaceHolder, int width, int height) {
            LogHelper.i(TAG, "[surfaceChanged] holder = " + surfaceHolder + " width = " + width +
                    " height = " + height);
            mPreviewSize = new Size(width, height);
            if (mCameraDevice != null) {
                mCameraDevice.updatePreviewSurface(surfaceHolder);
        ...
        public void surfaceDestroyed(SurfaceHolder surfaceHolder) {
            LogHelper.i(TAG, "[surfaceDestroyed] holder = " + surfaceHolder);
            if (mCameraDevice != null) {
