...
public class IntentVideoMode extends VideoMode {
    ...
    private static final int SIZE_2K = 1024 * 2;
    private static final double COMPUTE_PARAMETER = 0.95;
    ...
    protected IRecorder.RecorderSpec modifyRecorderSpec(
        ...
        if (mLimitSize > 0 && mLimitSize < mVideoHelper.getRecorderMaxSize()) {
            ...
            if (getProfile().quality == CamcorderProfile.QUALITY_LOW) {
                // M: enlarge recording video size nearby to limit size in MMS
                // in case of low quality and files size below 2M.
                // Why is 0.95 is design from media recorder native.
                // this function will delete and android O because google fix it.
                recorderSpec.maxFileSizeBytes
                        = (long) ((double) mLimitSize / COMPUTE_PARAMETER - SIZE_2K);
            }
        }
    ...
    protected IVideoUI.UISpec modifyUISpec(IVideoUI.UISpec spec) {
        if (mLimitSize > 0 && isSupportedExtendInfo()) {
    ...
    protected void addFileToMediaStore() {
        ...
        if (mVideoFileDescriptor != null) {
            ...
            mApp.getActivity().runOnUiThread(new Runnable() {
                @Override
                public void run() {
                    mAppUi.setUIVisibilityImmediately(IAppUi.PREVIEW_FRAME, View.GONE);
                }
            });
            ...
            ContentValues contentValues =
                    mVideoHelper.prepareContentValues(true, orientation, null,null);
            ...
            try {
                ...
                if (mLimitSize > 0) {
                    MediaRecorderEx.setParametersExtra(mRecorder.getMediaRecorder(),
                       mVideoHelper.RECORDER_INFO_SUFFIX + mVideoHelper.MEDIA_INFO_RECORDING_SIZE);
                }
