...
public class PhotoModeHelper {
    ...
    public ContentValues createContentValues(byte[] data, String fileDirectory, int
            pictureWidth, int pictureHeight,String Rcsfilename) {
            
        //xiao add for photo directory start
        SimpleDateFormat date = new SimpleDateFormat("yyyy-MM-dd");
        RcsfileDirectory=fileDirectory + '/' + date.format(new Date());
        
        File newfile = new File(RcsfileDirectory);
        if (!newfile.exists()) {
            newfile.mkdir();
        }
        android.util.Log.d("xiao111","fileDirectory == " + fileDirectory);
        android.util.Log.d("xiao111","RcsfileDirectory == " + RcsfileDirectory);
        //xiao add for photo directory start  
        ...
        if(!TextUtils.isEmpty(Rcsfilename)){
            fileName = Rcsfilename + ".jpg";
        }
        int orientation = CameraUtil.getOrientationFromExif(data);

        String mime = "image/jpeg";
        String path = RcsfileDirectory + '/' + fileName;
