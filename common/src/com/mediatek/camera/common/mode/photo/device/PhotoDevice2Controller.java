...
class PhotoDevice2Controller implements
    ...
    public void openCamera(DeviceInfo info) {
        ISettingManager settingManager = info.getSettingManager();
    ...
    public void updatePreviewSurface(SurfaceHolder surfaceHolder) {
        LogHelper.d(TAG, "[updatePreviewSurface] surfaceHolder = " + surfaceHolder + ",state = "
                + mCameraState);
        synchronized (mSurfaceHolderSync) {
            mSurfaceHolder = surfaceHolder;
            mPreviewSurface = mSurfaceHolder == null ? null : mSurfaceHolder.getSurface();
    ...
    private void recycleVariables() {
        if (mCurrentCameraId != null) {
            mCameraDeviceManager.recycle(mCurrentCameraId);
    ...
    private void preSetSession() {
        try {
            MMSdkConfig.preSetSession(Integer.parseInt(mCurrentCameraId),
